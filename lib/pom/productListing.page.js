...
class ListingPage extends Page { 
    ...
    /*async checkSizes (...args){
        ...
        first: for(let el in products){  
            let index = +el + 1
            let parentWindow = await this.selectQuickMenu(index)
            await this.driver.findElement(By.id("group_1")).click()
            let sizes = await this.driver.findElements(By.xpath("//select/option"))
            console.log(`Checking the ${index} product`)
            for(let size of sizes){
                ...
                for(let givenSize of args){
                    switch (checker.includes(givenSize)){
                        case true:
                            checkList.push(checker)
                            await this.driver.switchTo().window(parentWindow) // Swithcing frame
                            await this.driver.findElement(By.css('a[title="Close"]')).click()
                            continue first  
                    }
                }
            }
